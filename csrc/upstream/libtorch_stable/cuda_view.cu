...
torch::stable::Tensor get_cuda_view_from_cpu_tensor(
  ...
  if (torch::stable::detail::to<bool>(is_pinned_stack[0])) {
    ...
    hggcError_t err = hggcHostGetDevicePointer(&device_ptr, host_ptr, 0);
    STD_TORCH_CHECK(err == hggcSuccess, "cudaHostGetDevicePointer failed: ",
  ...
  hggcError_t err = hggcHostAlloc(&host_ptr, nbytes, hggcHostAllocMapped);
  if (err != hggcSuccess) {
    STD_TORCH_CHECK(false, "cudaHostAlloc failed: ", hggcGetErrorString(err));
  }

  err = hggcMemcpy(host_ptr, contiguous_cpu.const_data_ptr(), nbytes,
                   hggcMemcpyDefault);
  if (err != hggcSuccess) {
    hggcFreeHost(host_ptr);
    STD_TORCH_CHECK(false, "cudaMemcpy failed: ", hggcGetErrorString(err));
  }

  void* device_ptr = nullptr;
  err = hggcHostGetDevicePointer(&device_ptr, host_ptr, 0);
  if (err != hggcSuccess) {
    hggcFreeHost(host_ptr);
    STD_TORCH_CHECK(
        false, "cudaHostGetDevicePointer failed: ", hggcGetErrorString(err));
  }

  auto deleter = [host_ptr](void*) { hggcFreeHost(host_ptr); };
