...
typedef __hip_bfloat16_raw __ppu_bfloat16_raw;
  #if defined(HIP_FP8_TYPE_OCP)
typedef __hip_fp8_e4m3 __hg_fp8_e4m3;
typedef __hip_fp8x4_e4m3 __hg_fp8x4_e4m3;
  #else
// ROCm 6.2 fallback: only *_fnuz types exist
typedef __hip_fp8_e4m3_fnuz __hg_fp8_e4m3;
typedef __hip_fp8x4_e4m3_fnuz __hg_fp8x4_e4m3;
  #endif
#endif
...
__device__ __forceinline__ __ppu_bfloat162 silu2_v2(float2 x) {
#ifndef USE_ROCM
...
__device__ __forceinline__ __ppu_bfloat16 warp_max(__ppu_bfloat16 v) {
  static constexpr unsigned FULL_MASK = 0xffffffffu;
  for (int offset = 1; offset < WARP_SIZE; offset *= 2) {
...
#if __COMPATIBLECC_VER_MAJOR__ >= 11 && COMPATIBLE_ARCH >= 800
...
__device__ __forceinline__ __ppu_bfloat16 clip(__ppu_bfloat16 v,
                                              __ppu_bfloat16 mmin,
                                              __ppu_bfloat16 mmax) {
  return __hmin(mmax, __hmax(v, mmin));
}

__device__ __forceinline__ __ppu_bfloat162 clip(__ppu_bfloat162 v,
                                               __ppu_bfloat162 mmin,
                                               __ppu_bfloat162 mmax) {
...
template <class T>
constexpr __ppu_bfloat16 get_fp8_max() {
  static_assert(std::is_same_v<T, c10::Float8_e4m3fn> ||
                std::is_same_v<T, c10::Float8_e4m3fnuz>);
  if constexpr (std::is_same_v<T, c10::Float8_e4m3fn>) {
    return __ppu_bfloat16(__ppu_bfloat16_raw{.x = 17376});
  } else {
    return __ppu_bfloat16(__ppu_bfloat16_raw{.x = 17264});
  }
}

template <class T>
constexpr __ppu_bfloat16 get_fp8_min() {
  static_assert(std::is_same_v<T, c10::Float8_e4m3fn> ||
                std::is_same_v<T, c10::Float8_e4m3fnuz>);
  if constexpr (std::is_same_v<T, c10::Float8_e4m3fn>) {
    return __ppu_bfloat16(__ppu_bfloat16_raw{.x = 50144});
  } else {
    return __ppu_bfloat16(__ppu_bfloat16_raw{.x = 50032});
  }
}
...
__global__ void silu_mul_fp8_quant_deep_gemm_kernel(
    const __ppu_bfloat16* __restrict__ _input, fp8_type* __restrict__ _y_q,
  ...
  static constexpr __ppu_bfloat16 fp8_min = get_fp8_min<fp8_type>();
  static constexpr __ppu_bfloat16 fp8_max = get_fp8_max<fp8_type>();
  // We assign EPS with it's 16-bit unsigned counterpart to allow constexpr.
  static constexpr __ppu_bfloat16 EPS = (__ppu_bfloat16_raw{.x = 11996});
  int tid = threadIdx.x;
  int warp_id = tid >> 5;
  ...
  const __ppu_bfloat16 fp8_inv = __hdiv(__float2bfloat16(1.f), fp8_max);

  int32_t compute_pipeline_offset_64 = 0;
  int32_t load_stage_offset{};
  const __ppu_bfloat16 one_bf16 = __float2bfloat16_rn(1.f);
  ...
  __hg_fp8x4_e4m3* y_q_base_ptr =
      reinterpret_cast<__hg_fp8x4_e4m3*>(_y_q) + lane_id;
  ...
  for (auto j = tokens_lower; j < tokens_upper; j++) {
    ...
    for (int i = 0; i < COMPUTE_LIMIT; i++) {
      ...
      // Compute
      __ppu_bfloat162 res[2];
      __ppu_bfloat162* s_up_comp = reinterpret_cast<__ppu_bfloat162*>(&up64);
      __ppu_bfloat162* s_gate_comp = reinterpret_cast<__ppu_bfloat162*>(&gate64);

  #pragma unroll
      for (int32_t k = 0; k < 2; ++k) {
        __ppu_bfloat162 gate = silu2_v2(__bfloat1622float2(s_gate_comp[k]));
      ...
      __ppu_bfloat16 y_s = __hmul(warp_max(_y_max2.x), fp8_inv);

      if constexpr (CEIL_UE8M0) {
        y_s = hexp2(hceil(hlog2(y_s)));
      }

      __ppu_bfloat16 inv_y = __hdiv(one_bf16, y_s);
  ...
  #pragma unroll
      ...
      *y_q_ptr = __hg_fp8x4_e4m3(res[0], res[1]);
...
void persistent_masked_m_silu_mul_quant(
  ...
  const hggcStream_t stream = get_current_cuda_stream(input.get_device_index());
        ...
        y_q.scalar_type(), "silu_mul_fp8_quant_deep_gemm_kernel", [&] {        \
          vllm::silu_mul_fp8_quant_deep_gemm_kernel<                           \
              ...
              <<<grid, block, max_shared_mem_bytes + (E + 1) * 16, stream>>>(  \
                  reinterpret_cast<const __ppu_bfloat16*>(                      \
