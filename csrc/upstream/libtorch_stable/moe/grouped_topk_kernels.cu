...
template <>
__device__ inline float cuda_cast<float, __ppu_bfloat16>(__ppu_bfloat16 val) {
...
__device__ inline T neg_inf() {
  ...
  return cuda_cast<T, float>(-hggc::std::numeric_limits<float>::infinity());
}

template <typename T>
__device__ inline bool is_finite(const T val) {
#if (__COMPATIBLECC_VER_MAJOR__ * 10000 + __COMPATIBLECC_VER_MINOR__ * 100 >= 120800)
...
__global__ void grouped_topk_fused_kernel(
  ...
  hggcGridDependencySynchronize();  // I think all prolog can be put before
...
void launch(T* scores, float* topk_values, IdxT* topk_indices,
            ...
            bool enable_pdl, hggcLaunchConfig_t& config) {
...
bool dispatch(TierList<>*, T*, float*, IdxT*, BiasT const*, int64_t, int64_t,
              int64_t, bool, double, bool, hggcLaunchConfig_t&) {
  return false;
}
...
void invokeNoAuxTc(T* scores, float* topk_values, IdxT* topk_indices,
  ...
  if (is_single_group || is_multi_group) {
    ...
    config.dynamicSmemBytes = 0;
    hggcLaunchKernelEx(&config, kernel_instance, scores, topk_values,
...
#define INSTANTIATE_NOAUX_TC(T, BiasT, IdxT, SF)                             \
  template void invokeNoAuxTc<T, BiasT, IdxT, SF>(                           \
      ...
      bool const renormalize, double const routed_scaling_factor,            \
      const bool enable_pdl, hggcStream_t const stream);

INSTANTIATE_NOAUX_TC(float, float, int32_t, SCORING_SIGMOID);
INSTANTIATE_NOAUX_TC(float, half, int32_t, SCORING_SIGMOID);
INSTANTIATE_NOAUX_TC(float, __ppu_bfloat16, int32_t, SCORING_SIGMOID);
INSTANTIATE_NOAUX_TC(half, float, int32_t, SCORING_SIGMOID);
INSTANTIATE_NOAUX_TC(half, half, int32_t, SCORING_SIGMOID);
INSTANTIATE_NOAUX_TC(half, __ppu_bfloat16, int32_t, SCORING_SIGMOID);
INSTANTIATE_NOAUX_TC(__ppu_bfloat16, float, int32_t, SCORING_SIGMOID);
INSTANTIATE_NOAUX_TC(__ppu_bfloat16, half, int32_t, SCORING_SIGMOID);
INSTANTIATE_NOAUX_TC(__ppu_bfloat16, __ppu_bfloat16, int32_t, SCORING_SIGMOID);
INSTANTIATE_NOAUX_TC(float, float, int32_t, SCORING_NONE);
INSTANTIATE_NOAUX_TC(float, half, int32_t, SCORING_NONE);
INSTANTIATE_NOAUX_TC(float, __ppu_bfloat16, int32_t, SCORING_NONE);
INSTANTIATE_NOAUX_TC(half, float, int32_t, SCORING_NONE);
INSTANTIATE_NOAUX_TC(half, half, int32_t, SCORING_NONE);
INSTANTIATE_NOAUX_TC(half, __ppu_bfloat16, int32_t, SCORING_NONE);
INSTANTIATE_NOAUX_TC(__ppu_bfloat16, float, int32_t, SCORING_NONE);
INSTANTIATE_NOAUX_TC(__ppu_bfloat16, half, int32_t, SCORING_NONE);
INSTANTIATE_NOAUX_TC(__ppu_bfloat16, __ppu_bfloat16, int32_t, SCORING_NONE);
}  // end namespace moe
}  // namespace vllm
...
#define LAUNCH_KERNEL(T, IdxT)                                         \
  do {                                                                 \
    switch (bias_type) {                                               \
      ...
      case torch::headeronly::ScalarType::BFloat16:                    \
        LAUNCH_KERNEL_SF(T, __ppu_bfloat16, IdxT);                      \
        break;                                                         \
      default:                                                         \
        STD_TORCH_CHECK(                                               \
            false,                                                     \
  ...
  switch (data_type) {
    ...
    case torch::headeronly::ScalarType::BFloat16:
      // Handle BFloat16
      LAUNCH_KERNEL(__ppu_bfloat16, int32_t);
      break;
    default:
