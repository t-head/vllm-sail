...
{
    static_assert(std::is_same_v<InputType, float> || std::is_same_v<InputType, __ppu_bfloat16> ||
    ...
    if constexpr (std::is_same_v<InputType, __ppu_bfloat16> || std::is_same_v<InputType, __half>) {
...
#pragma unroll
            for (int ii = 0; ii < LDG_PER_THREAD; ++ii) {
                const __ppu_bfloat16* scalar_ptr = thread_read_ptr + ii * THREADS_PER_ROW;
...
void topkGatingLauncherHelper(const InputType* input, const bool* finished, float* output, IndType* indices,
    ...
    const float* bias, const double routed_scaling_factor, hggcStream_t stream, const bool* is_padding)
{
...
void topkGatingKernelLauncher(
    ...
    (std::is_same_v<InputType, __ppu_bfloat16> || std::is_same_v<InputType, __half>) ? 4 : 8;
#endif
    switch (num_experts) {
        case 1:
    ...
    std::optional<torch::stable::Tensor> is_padding)
{
    ...
    if (gating_output.scalar_type() == torch::headeronly::ScalarType::Float) {
        ...
        dispatch_topk_launch<__ppu_bfloat16, vllm::moe::SCORING_SOFTMAX>(gating_output, topk_weights, topk_indices,
            token_expert_indices, softmax_workspace, num_tokens, num_experts, topk, renormalize,
            bias, 1.0, stream, is_padding);
    } else {
    ...
    std::optional<torch::stable::Tensor> is_padding)
{
    ...
    if (gating_output.scalar_type() == torch::headeronly::ScalarType::Float) {
        ...
        dispatch_topk_launch<__ppu_bfloat16, vllm::moe::SCORING_SIGMOID>(gating_output, topk_weights, topk_indices,
            token_expert_indices, workspace, num_tokens, num_experts, topk, renormalize,
            bias, routed_scaling_factor, stream, is_padding);
    } else {
        STD_TORCH_CHECK(false, "Unsupported gating_output data type: ", gating_output.scalar_type());
