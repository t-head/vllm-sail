...
void launchDsv4HashTopk(const float* input, float* output, OutIndType* indices,
                        ...
                        const HashIndType* input_ids,
                        const HashIndType* tid2eid, hggcStream_t stream,
                        const bool* is_padding, const float* bias_vl,
                        int64_t image_sentinel_lo) {
  ...
  hggcLaunchKernelEx(&config, kernel, input, output, indices, num_rows,
...
__launch_bounds__(WARPS_PER_CTA* WARP_SIZE_PARAM) __global__
  ...
  static_assert(std::is_same_v<InputType, float> ||
                    std::is_same_v<InputType, __ppu_bfloat16> ||
  ...
  if constexpr (std::is_same_v<InputType, __ppu_bfloat16> ||
                std::is_same_v<InputType, __half>) {
...
#pragma unroll
  ...
  } else if constexpr (std::is_same_v<InputType, __ppu_bfloat16>) {
    if constexpr (ELTS_PER_LDG >= 2) {
      using VecType = AlignedArray<__ppu_bfloat16, ELTS_PER_LDG>;
...
#pragma unroll
        for (int jj = 0; jj < ELTS_PER_LDG / 2; ++jj) {
          row_chunk_f2[base_idx_f2 + jj] = __bfloat1622float2(
              *reinterpret_cast<const __ppu_bfloat162*>(vec.data + jj * 2));
        }
      }
    } else {  // ELTS_PER_LDG == 1
#pragma unroll
      for (int ii = 0; ii < LDG_PER_THREAD; ++ii) {
        const __ppu_bfloat16* scalar_ptr =
...
void topkGatingSoftplusSqrtLauncherHelper(
    ...
    const HashIndType* tid2eid, hggcStream_t stream, const bool* is_padding,
  ...
  DISPATCH_HASH(use_hash, USE_HASH, {
    ...
    config.numAttrs = 1;
    config.attrs = attrs;
    hggcLaunchKernelEx(&config, kernel, input, finished, output, num_rows,
...
void topkGatingSoftplusSqrtKernelLauncher(
    ...
    const HashIndType* input_ids, const HashIndType* tid2eid,
    hggcStream_t stream, const bool* is_padding, const float* bias_vl,
    int64_t image_sentinel_lo) {
#ifndef USE_ROCM
...
void dispatch_topk_softplus_sqrt_launch(
    ...
    const std::optional<torch::stable::Tensor>& tid2eid, hggcStream_t stream,
...
void topk_softplus_sqrt(
  ...
  if (gating_output.scalar_type() == torch::headeronly::ScalarType::Float) {
    ...
    dispatch_topk_softplus_sqrt_launch<__ppu_bfloat16>(
        reinterpret_cast<const __ppu_bfloat16*>(gating_output.const_data_ptr()),
        topk_weights, topk_indices, token_expert_indices, num_tokens,
        num_experts, topk, renormalize, routed_scaling_factor, correction_bias,
        input_ids, tid2eid, stream, is_padding, bias_vl, image_sentinel_lo);
  } else {
