...
void launch_persistent_topk(const torch::stable::Tensor& logits,
  ...
  if (num_sms == 0) {
    const hggcDeviceProp* device_prop = get_device_prop();
  ...
  if (num_rows > 32 && max_smem_per_block >= 128 * 1024) {
    ...
    STD_TORCH_CHECK(status == hggcSuccess,
                    "FilteredTopK failed: ", hggcGetErrorString(status));
  } else {
    ...
    int occupancy = 1;
    hggcError_t occ_err = hggcSuccess;
    if (vec_size == 4) {
      occ_err = hggcOccupancyMaxActiveBlocksPerMultiprocessor(
    ...
    STD_TORCH_CHECK(occ_err == hggcSuccess,
                    "persistent_topk occupancy query failed: ",
                    hggcGetErrorString(occ_err));
    ...
    if (needs_cooperative && total_ctas > hw_resident_cap) {
      ...
      STD_TORCH_CHECK(status == hggcSuccess, "FilteredTopK fallback failed: ",
    ...
    {
      hggcError_t mz_err = hggcMemsetAsync(
          workspace.mutable_data_ptr<uint8_t>(), 0, state_bytes, stream);
      STD_TORCH_CHECK(mz_err == hggcSuccess,
                      "row_states memset failed: ", hggcGetErrorString(mz_err));
    }
  ...
  #define LAUNCH_PERSISTENT(TOPK_VAL, VS)                                     \
    do {                                                                      \
      ...
      hggcError_t err = hggcFuncSetAttribute(                                 \
          kernel, hggcFuncAttributeMaxDynamicSharedMemorySize, smem_size);    \
      STD_TORCH_CHECK(err == hggcSuccess,                                     \
                      "Failed to set smem: ", hggcGetErrorString(err));       \
  ...
  STD_TORCH_CHECK(err == hggcSuccess,
                  "persistent_topk failed: ", hggcGetErrorString(err));
}
#endif
