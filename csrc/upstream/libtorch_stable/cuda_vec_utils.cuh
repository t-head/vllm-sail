...
#if !defined(USE_ROCM) && defined(COMPATIBLE_ARCH) && COMPATIBLE_ARCH >= 1000 && \
    defined(COMPATIBLE_RT_VERSION) && COMPATIBLE_RT_VERSION >= 12090
...
template <>
struct PackedTypeConverter<__ppu_bfloat162> {
  using Type = __ppu_bfloat16;
};

template <>
struct PackedTypeConverter<__ppu_bfloat16> {
...
__device__ __forceinline__ float2 cast_to_float2(const packed_t& val) {
  if constexpr (std::is_same_v<packed_t, __ppu_bfloat162>) {
