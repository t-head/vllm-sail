...
__device__ __forceinline__ uint4 packFp8E4M3x16(float const* values,
...
#ifndef USE_ROCM
  uint4 out;
  auto* out2 = reinterpret_cast<__hg_fp8x2_storage_t*>(&out);
  #pragma unroll
  for (int i = 0; i < kElemsPerLane / 2; i++) {
    ...
    out2[i] = __hg_cvt_float2_to_fp8x2(scaled, __HG_SATFINITE, __HG_E4M3);
  }
  return out;
#else
...
#pragma unroll
        for (int i = 0; i < kElemsPerLane; i++) {
          ...
          scaled = fminf(fmaxf(scaled, -kFp8Max), kFp8Max);
#ifndef USE_ROCM
          __hg_fp8_storage_t s =
              __hg_cvt_float_to_fp8(scaled, __HG_SATFINITE, __HG_E4M3);
...
#if (!defined(COMPATIBLE_ARCH) || COMPATIBLE_ARCH < 800) && !defined(USE_ROCM)
...
void launchFusedDeepseekV4QNormRopeKVRopeQuantInsert(
    ...
    bool const apply_q_norm, bool const kv_mxfp8, hggcStream_t stream) {
#define DISPATCH(N)                                                         \
  case N:                                                                   \
    if (apply_q_norm) {                                                     \
...
static void launchFullCacheKernel(
    ...
    char const* op_name, hggcStream_t stream) {
  constexpr int kBlockSize = 256;
  constexpr int kWarpsPerBlock = kBlockSize / 32;
  int64_t const total_warps =
  ...
  config.attrs = attrs;
  config.numAttrs = (sm_version >= 90) ? 1 : 0;
  hggcLaunchKernelEx(&config, kernel, q_inout, q_fp8_out, q_fp8_stride0,
...
torch::stable::Tensor fused_deepseek_v4_qnorm_rope_kv_rope_quant_insert(
  ...
  const hggcStream_t stream = get_current_cuda_stream(q_in.get_device_index());
...
void fused_deepseek_v4_qnorm_rope_kv_rope_full_cache_bf16_insert(
  ...
  const torch::stable::accelerator::DeviceGuard device_guard(
      q.get_device_index());
  const hggcStream_t stream = get_current_cuda_stream(q.get_device_index());
