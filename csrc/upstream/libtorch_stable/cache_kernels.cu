...
void swap_blocks(torch::stable::Tensor& src, torch::stable::Tensor& dst,
  ...
  hggcMemcpyKind memcpy_type;
  if (src_device.is_cuda() && dst_device.is_cuda()) {
    STD_TORCH_CHECK(src_device.index() == dst_device.index(),
                    "src and dst must be on the same GPU");
    memcpy_type = hggcMemcpyDeviceToDevice;
  } else if (src_device.is_cuda() && dst_device.is_cpu()) {
    memcpy_type = hggcMemcpyDeviceToHost;
  } else if (src_device.is_cpu() && dst_device.is_cuda()) {
    memcpy_type = hggcMemcpyHostToDevice;
  } else {
  ...
  for (size_t i = 0; i < num_blocks; i++) {
    ...
    hggcMemcpyAsync(dst_ptr + dst_offset, src_ptr + src_offset,
...
void swap_blocks_batch(const torch::stable::Tensor& src_ptrs,
  ...
  static_assert(sizeof(HGdeviceptr) == sizeof(int64_t));
  ...
  using BatchFn =
      HGresult (*)(HGdeviceptr*, HGdeviceptr*, size_t*, size_t,
                   HGmemcpyAttributes*, size_t*, size_t, size_t*, HGstream);
  static BatchFn batch_fn = []() -> BatchFn {
    HGdriverProcAddressQueryResult sym_status;
    void* fn_ptr = nullptr;
    HGresult res = hgGetProcAddress("cuMemcpyBatchAsync", &fn_ptr, 12080,
                                    HG_GET_PROC_ADDRESS_DEFAULT, &sym_status);
    if (res != HGGC_SUCCESS || fn_ptr == nullptr) {
      return nullptr;
    }
  ...
  const bool usable_stream = stream != nullptr && stream != hggcStreamLegacy;
  if (batch_fn != nullptr && usable_stream) {
    HGmemcpyAttributes attr = {};
    ...
    attr.srcAccessOrder = is_src_access_order_any
                              ? HG_MEMCPY_SRC_ACCESS_ORDER_ANY
                              : HG_MEMCPY_SRC_ACCESS_ORDER_STREAM;
    size_t attrs_idx = 0;
    size_t fail_idx = 0;
    ...
    for (int64_t off = 0; off < n; off += step) {
      ...
      HGresult result = batch_fn(reinterpret_cast<HGdeviceptr*>(dst_data + off),
                                 reinterpret_cast<HGdeviceptr*>(src_data + off),
                                 reinterpret_cast<size_t*>(size_data + off),
                                 static_cast<size_t>(cnt), &attr, &attrs_idx, 1,
                                 &fail_idx, static_cast<HGstream>(stream));
      STD_TORCH_CHECK(result == HGGC_SUCCESS,
...
#endif
  {
    ...
    for (int64_t i = 0; i < n; i++) {
      hggcMemcpyAsync(reinterpret_cast<void*>(dst_data[i]),
                      reinterpret_cast<void*>(src_data[i]),
                      static_cast<size_t>(size_data[i]), hggcMemcpyDefault,
                      stream);
    }
  }
}
...
#if !defined(USE_ROCM) && defined(ENABLE_NVFP4_SM100) && ENABLE_NVFP4_SM100
    const hggcDeviceProp* props = get_device_prop();
...
#define LAUNCH_GROUPED_FP8(KV_DTYPE)                                           \
  vllm::concat_and_cache_mla_grouped_kernel<__ppu_bfloat16, uint8_t, KV_DTYPE>  \
      <<<grid, block, 0, stream>>>(                                            \
          reinterpret_cast<const __ppu_bfloat16*>(kv_c.data_ptr()),             \
          reinterpret_cast<const __ppu_bfloat16*>(k_pe.data_ptr()),             \
...
void convert_fp8(torch::stable::Tensor& dst_cache,
  ...
  if (kv_cache_dtype == "auto") {
    if (src_cache.scalar_type() == torch::headeronly::ScalarType::Float) {
      ...
      CALL_CONVERT_FP8(uint8_t, __ppu_bfloat16, vllm::Fp8KVCacheDataType::kAuto);
      ...
      CALL_CONVERT_FP8(uint16_t, uint8_t, vllm::Fp8KVCacheDataType::kAuto);
               ...
               torch::headeronly::ScalarType::BFloat16) {
      CALL_CONVERT_FP8(__ppu_bfloat16, uint8_t, vllm::Fp8KVCacheDataType::kAuto);
    }
    ...
    if (src_cache.scalar_type() == torch::headeronly::ScalarType::Float) {
      ...
      CALL_CONVERT_FP8(uint8_t, uint16_t, vllm::Fp8KVCacheDataType::kFp8E4M3);
               ...
               torch::headeronly::ScalarType::BFloat16) {
      CALL_CONVERT_FP8(uint8_t, __ppu_bfloat16,
      ...
      CALL_CONVERT_FP8(uint16_t, uint8_t, vllm::Fp8KVCacheDataType::kFp8E4M3);
               ...
               torch::headeronly::ScalarType::BFloat16) {
      CALL_CONVERT_FP8(__ppu_bfloat16, uint8_t,
                       vllm::Fp8KVCacheDataType::kFp8E4M3);
    }
  } else {
...
__device__ __forceinline__ void gather_and_upconvert_fp8_token(
    const uint8_t* __restrict__ token_ptr, __ppu_bfloat16* __restrict__ dst_ptr,
    int32_t lane_id) {
...
#else
    const bf16_8_t bf16_data =
        fp8::scaled_vec_conversion<bf16_8_t, uint2>(fp8_data, scale, __HG_E4M3);
#endif
...
__global__ void cp_gather_and_upconvert_fp8_kv_cache_page(
    const uint8_t* __restrict__ src_cache, __ppu_bfloat16* __restrict__ dst,
    const int32_t* __restrict__ block_table,
  ...
  for (int32_t page_token = page.page_token_begin + warp_id;
    ...
    __ppu_bfloat16* dst_ptr = dst + output_token * dst_entry_stride;
...
void cp_gather_and_upconvert_fp8_kv_cache(
  ...
  if (has_host_rows) {
    ...
    STD_TORCH_CHECK(
        ...
        "src_cache");
    hggcPointerAttributes attributes{};
    const hggcError_t pointer_status =
        hggcPointerGetAttributes(&attributes, host.const_data_ptr());
    if (pointer_status != hggcSuccess) {
      hggcGetLastError();
    }
    STD_TORCH_CHECK(
        pointer_status == hggcSuccess && attributes.type == hggcMemoryTypeHost,
  ...
  vllm::cp_gather_and_upconvert_fp8_kv_cache_page<<<
      ...
      src_ptr, reinterpret_cast<__ppu_bfloat16*>(dst.data_ptr()),
      block_table.const_data_ptr<int32_t>(),
