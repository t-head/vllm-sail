...
void swap_blocks_batch(const torch::stable::Tensor& src_ptrs,
  ...
  if (batch_fn != nullptr && usable_stream) {
    ...
    size_t attrs_idx = 0;
    size_t fail_idx = 0;
    HGresult result = batch_fn(reinterpret_cast<HGdeviceptr*>(dst_data),
                               reinterpret_cast<HGdeviceptr*>(src_data),
...
__global__ void cp_gather_and_upconvert_fp8_kv_cache(
    ...
    __ppu_bfloat16* __restrict__ dst,          // [total_tokens, 576]
...
#else
  const bf16_8_t bf16_lo =
      fp8::scaled_vec_conversion<bf16_8_t, uint2>(fp8_lo, scale, __HG_E4M3);
  const bf16_8_t bf16_hi =
      fp8::scaled_vec_conversion<bf16_8_t, uint2>(fp8_hi, scale, __HG_E4M3);
#endif

  __ppu_bfloat16* dst_ptr = dst + out_token_id * dst_entry_stride;
