...
#ifndef USE_ROCM
      uint2 out;
      auto* o2 = reinterpret_cast<__hg_fp8x2_storage_t*>(&out);
  #pragma unroll
      for (int i = 0; i < 4; i++) {
        ...
        o2[i] = __hg_cvt_float2_to_fp8x2(s, __HG_SATFINITE, __HG_E4M3);
      }
...
__device__ __forceinline__ void copyChunk8UnitFp8(uint8_t* dst,
  ...
  if constexpr (!Converter::exists) {
    ...
    uint2 output;
    auto* output2 = reinterpret_cast<__hg_fp8x2_storage_t*>(&output);
  #pragma unroll
    for (int i = 0; i < 4; ++i) {
      if constexpr (std::is_same_v<scalar_t, c10::BFloat16>) {
        output2[i] = __hg_cvt_bfloat16raw2_to_fp8x2(
            static_cast<__ppu_bfloat162_raw>(input2[i]), __HG_SATFINITE,
            __HG_E4M3);
      } else {
        output2[i] = __hg_cvt_halfraw2_to_fp8x2(
            static_cast<__half2_raw>(input2[i]), __HG_SATFINITE, __HG_E4M3);
      }
    }
...
static void launchPdlSlots(KernelT kernel, int num_tokens, int slots_per_token,
...
#if !defined(USE_ROCM) && !defined(__HGGCCC__)
  ...
  hggcLaunchKernelEx(&config, kernel, args...);
#else
  // clang-format off
