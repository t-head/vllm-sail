...
#define LAUNCH_ACTIVATION_GATE_KERNEL(KERNEL, PACKED_KERNEL, ACT_FIRST,        \
  ...
  int support_vec =                                                            \
      (COMPATIBLE_VERSION >= 12090 && cc_major >= 10 && num_tokens > 128)            \
  ...
  if (use_vec) {                                                               \
    ...
    if (COMPATIBLE_VERSION >= 12090 && cc_major >= 10 && num_tokens > 128) {         \
...
#if !defined(USE_ROCM) && defined(COMPATIBLE_ARCH) && COMPATIBLE_ARCH >= 800
  return c10::Float8_e4m3fn(__hg_cvt_float_to_fp8(x, __HG_SATFINITE, __HG_E4M3),
