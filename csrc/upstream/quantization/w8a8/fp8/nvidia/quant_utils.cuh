...
__inline__ __device__ Tout vec_conversion(
    const Tin& x, const __hg_fp8_interpretation_t fp8_type = __HG_E4M3) {
...
vec_conversion<c10::Float8_e4m3fn, float>(
    const float& a, const __hg_fp8_interpretation_t fp8_type) {
    #if defined(COMPATIBLE_ARCH) && COMPATIBLE_ARCH < 800
  return static_cast<c10::Float8_e4m3fn>(a);
    #else
  return c10::Float8_e4m3fn(__hg_cvt_float_to_fp8(a, __HG_SATFINITE, fp8_type),
...
__inline__ __device__ uint16_t vec_conversion<uint16_t, uint8_t>(
    const uint8_t &a, const __hg_fp8_interpretation_t fp8_type) {
  __half_raw res = __hg_cvt_fp8_to_halfraw(a, fp8_type);
  return res.x;
}
...
__inline__ __device__ uint32_t vec_conversion<uint32_t, uint16_t>(
    const uint16_t &a, const __hg_fp8_interpretation_t fp8_type) {
  union {
    uint16_t u16[2];
    uint32_t u32;
  } tmp;
  __half2_raw res = __hg_cvt_fp8x2_to_halfraw2(a, fp8_type);
  tmp.u16[0] = res.x;
  tmp.u16[1] = res.y;
  return tmp.u32;
}
...
__inline__ __device__ uint2 vec_conversion<uint2, uint32_t>(
    const uint32_t &a, const __hg_fp8_interpretation_t fp8_type) {
  union {
    uint2 u32x2;
    uint32_t u32[2];
  } tmp;
...
__inline__ __device__ uint4 vec_conversion<uint4, uint2>(
    const uint2 &a, const __hg_fp8_interpretation_t fp8_type) {
  union {
    uint4 u64x2;
    uint2 u64[2];
  } tmp;
...
// fp8 -> __nv_bfloat16
template <>
__inline__ __device__ __ppu_bfloat16 vec_conversion<__ppu_bfloat16, uint8_t>(
    const uint8_t &a, const __hg_fp8_interpretation_t fp8_type) {
...
template <>
__inline__ __device__ __ppu_bfloat162 vec_conversion<__ppu_bfloat162, uint16_t>(
    const uint16_t &a, const __hg_fp8_interpretation_t fp8_type) {
  __ppu_bfloat162 res;
  res.x = vec_conversion<__ppu_bfloat16, uint8_t>((uint8_t)a, fp8_type);
  res.y = vec_conversion<__ppu_bfloat16, uint8_t>((uint8_t)(a >> 8U), fp8_type);
  return res;
}
...
__inline__ __device__ bf16_4_t vec_conversion<bf16_4_t, uint32_t>(
    const uint32_t &a, const __hg_fp8_interpretation_t fp8_type) {
  bf16_4_t res;
  res.x = vec_conversion<__ppu_bfloat162, uint16_t>((uint16_t)a, fp8_type);
  res.y =
      vec_conversion<__ppu_bfloat162, uint16_t>((uint16_t)(a >> 16U), fp8_type);
  return res;
}
...
vec_conversion<float, uint8_t>(const uint8_t &a,
                               const __hg_fp8_interpretation_t fp8_type) {
  // fp8 -> half
...
__inline__ __device__ uint8_t vec_conversion<uint8_t, uint16_t>(
  ...
  __half_raw tmp;
  tmp.x = a;
  __hg_fp8_storage_t res =
      __hg_cvt_halfraw_to_fp8(tmp, __HG_SATFINITE, fp8_type);
  return (uint8_t)res;
}

// bf16 -> fp8
template <>
__inline__ __device__ uint8_t vec_conversion<uint8_t, __ppu_bfloat16>(
    const __ppu_bfloat16 &a, const __hg_fp8_interpretation_t fp8_type) {
      #if defined(COMPATIBLE_ARCH) && COMPATIBLE_ARCH < 800
  assert(false);
      #else
  __hg_fp8_storage_t res = __hg_cvt_bfloat16raw_to_fp8(
      __ppu_bfloat16_raw(a), __HG_SATFINITE, fp8_type);
  return (uint8_t)res;
      #endif
}
...
__inline__ __device__ uint8_t vec_conversion<uint8_t, float>(
    const float &a, const __hg_fp8_interpretation_t fp8_type) {
  __hg_fp8_storage_t res = __hg_cvt_float_to_fp8(a, __HG_SATFINITE, fp8_type);
  return (uint8_t)res;
}
...
__inline__ __device__ uint32_t vec_conversion<uint32_t, float2>(
    const float2 &a, const __hg_fp8_interpretation_t fp8_type) {
  union {
    half2 float16;
    uint32_t uint32;
  };
...
__inline__ __device__ uint2 vec_conversion<uint2, Float4_>(
    const Float4_ &a, const __hg_fp8_interpretation_t fp8_type) {
  uint2 b;
  float2 val;
  val.x = a.x.x;
  val.y = a.x.y;
...
__inline__ __device__ uint4 vec_conversion<uint4, Float8_>(
    const Float8_ &a, const __hg_fp8_interpretation_t fp8_type) {
  uint4 b;
...
template <>
__inline__ __device__ __ppu_bfloat162 vec_conversion<__ppu_bfloat162, float2>(
    const float2 &a, const __hg_fp8_interpretation_t fp8_type) {
  __ppu_bfloat162 b;
  from_float(b, a);
  return b;
...
__inline__ __device__ Tout scaled_vec_conversion(
    const Tin& x, const float scale, const __hg_fp8_interpretation_t fp8_type) {
  static_assert(
...
__inline__ __device__ uint16_t scaled_vec_conversion<uint16_t, uint8_t>(
    ...
    const __hg_fp8_interpretation_t fp8_type) {
  __half_raw tmp = __hg_cvt_fp8_to_halfraw(a, fp8_type);
...
// fp8 -> __nv_bfloat16
template <>
__inline__ __device__ __ppu_bfloat16
scaled_vec_conversion<__ppu_bfloat16, uint8_t>(
    const uint8_t& a, const float scale,
    const __hg_fp8_interpretation_t fp8_type) {
  // Note there is no direct convert function from fp8 to bf16.
  // fp8 -> half
...
// fp8x2 -> __nv_bfloat162
template <>
__inline__ __device__ __ppu_bfloat162
scaled_vec_conversion<__ppu_bfloat162, uint16_t>(
    const uint16_t& a, const float scale,
    const __hg_fp8_interpretation_t fp8_type) {
  __ppu_bfloat162 res;
  res.x = scaled_vec_conversion<__ppu_bfloat16, uint8_t>((uint8_t)a, scale,
                                                        fp8_type);
  res.y = scaled_vec_conversion<__ppu_bfloat16, uint8_t>((uint8_t)(a >> 8U),
                                                        scale, fp8_type);
  return res;
}
...
__inline__ __device__ bf16_4_t scaled_vec_conversion<bf16_4_t, uint32_t>(
    ...
    const __hg_fp8_interpretation_t fp8_type) {
  bf16_4_t res;
  res.x = scaled_vec_conversion<__ppu_bfloat162, uint16_t>((uint16_t)a, scale,
                                                          fp8_type);
  res.y = scaled_vec_conversion<__ppu_bfloat162, uint16_t>((uint16_t)(a >> 16U),
                                                          scale, fp8_type);
  return res;
}
...
__inline__ __device__ uint8_t scaled_vec_conversion<uint8_t, uint16_t>(
    ...
    const __hg_fp8_interpretation_t fp8_type) {
  __hg_fp8_storage_t res =
      __hg_cvt_float_to_fp8(half_to_float(a) / scale, __HG_SATFINITE, fp8_type);
  return (uint8_t)res;
}

// bf16 -> fp8
template <>
__inline__ __device__ uint8_t scaled_vec_conversion<uint8_t, __ppu_bfloat16>(
    const __ppu_bfloat16& a, const float scale,
    const __hg_fp8_interpretation_t fp8_type) {
    #if defined(COMPATIBLE_ARCH) && COMPATIBLE_ARCH < 800
  assert(false);
    #else
  __hg_fp8_storage_t res = __hg_cvt_float_to_fp8(__bfloat162float(a) / scale,
                                                 __HG_SATFINITE, fp8_type);
  return (uint8_t)res;
    #endif
...
__inline__ __device__ uint8_t scaled_vec_conversion<uint8_t, c10::BFloat16>(
    ...
    const __hg_fp8_interpretation_t fp8_type) {
  return scaled_vec_conversion<uint8_t, __ppu_bfloat16>(
      reinterpret_cast<const __ppu_bfloat16&>(a), scale, fp8_type);
}
...
__inline__ __device__ uint8_t scaled_vec_conversion<uint8_t, float>(
    ...
    const __hg_fp8_interpretation_t fp8_type) {
  __hg_fp8_storage_t res =
      __hg_cvt_float_to_fp8(a / scale, __HG_SATFINITE, fp8_type);
  return (uint8_t)res;
}
...
__inline__ __device__ Tout convert(const Tin& x) {
  ...
  if constexpr (kv_dt == Fp8KVCacheDataType::kFp8E4M3) {
    return vec_conversion<Tout, Tin>(x, __HG_E4M3);
  } else if constexpr (kv_dt == Fp8KVCacheDataType::kFp8E5M2) {
    return vec_conversion<Tout, Tin>(x, __HG_E5M2);
  }
  #endif
  assert(false);
  __builtin_unreachable();  // Suppress missing return statement warning
...
__inline__ __device__ Tout scaled_convert(const Tin& x, const float scale) {
  #ifdef ENABLE_FP8
  if constexpr (kv_dt == Fp8KVCacheDataType::kFp8E4M3) {
    return scaled_vec_conversion<Tout, Tin>(x, scale, __HG_E4M3);
  } else if constexpr (kv_dt == Fp8KVCacheDataType::kFp8E5M2) {
    return scaled_vec_conversion<Tout, Tin>(x, scale, __HG_E5M2);
  }
  #endif
  assert(false);
  __builtin_unreachable();  // Suppress missing return statement warning
  ...
  #define DISPATCH_BY_KV_CACHE_DTYPE(SRC_DTYPE, KV_DTYPE, FN)                  \
    ...
    if (KV_CACHE_DTYPE == vllm::Fp8KVCacheDataType::kAuto) {                   \
      if (SRC_DTYPE == torch::headeronly::ScalarType::Float) {                 \
        ...
        FN(__ppu_bfloat16, __ppu_bfloat16, vllm::Fp8KVCacheDataType::kAuto);     \
      } else {                                                                 \
      ...
      if (SRC_DTYPE == torch::headeronly::ScalarType::Float) {                 \
        ...
        FN(__ppu_bfloat16, uint8_t, vllm::Fp8KVCacheDataType::kFp8E4M3);        \
      } else {                                                                 \
        STD_TORCH_CHECK(false,                                                 \
                        "Unsupported input type of kv cache: ", SRC_DTYPE);    \
      }                                                                        \
      ...
      if (SRC_DTYPE == torch::headeronly::ScalarType::Float) {                 \
        ...
        FN(__ppu_bfloat16, uint8_t, vllm::Fp8KVCacheDataType::kFp8E5M2);        \
      } else {                                                                 \
        STD_TORCH_CHECK(false,                                                 \
                        "Unsupported input type of kv cache: ", SRC_DTYPE);    \
      }                                                                        \
