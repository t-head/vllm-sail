...
using __ppu_bfloat16 = __hip_bfloat16;

// fp8 -> __nv_bfloat16
template <>
__inline__ __device__ __ppu_bfloat16
vec_conversion<__ppu_bfloat16, uint8_t>(const uint8_t& a) {
  fp8_type f8;
  f8.__x = a;
  return __float2bfloat16(static_cast<float>(f8));
}

using __ppu_bfloat162 = __hip_bfloat162;

// fp8x2 -> __nv_bfloat162
template <>
__inline__ __device__ __ppu_bfloat162
vec_conversion<__ppu_bfloat162, uint16_t>(const uint16_t& a) {
  __ppu_bfloat162 res;
  res.x = vec_conversion<__ppu_bfloat16, uint8_t>((uint8_t)a);
  res.y = vec_conversion<__ppu_bfloat16, uint8_t>((uint8_t)(a >> 8U));
  return res;
}
...
vec_conversion<bf16_4_t, uint32_t>(const uint32_t& a) {
  bf16_4_t res;
  res.x = vec_conversion<__ppu_bfloat162, uint16_t>((uint16_t)a);
  res.y = vec_conversion<__ppu_bfloat162, uint16_t>((uint16_t)(a >> 16U));
  return res;
}
...
template <>
__inline__ __device__ uint8_t
vec_conversion<uint8_t, __ppu_bfloat16>(const __ppu_bfloat16& a) {
...
template <>
__inline__ __device__ __ppu_bfloat162
vec_conversion<__ppu_bfloat162, float2>(const float2& a) {
  __ppu_bfloat162 b = __float22bfloat162_rn(a);
  return b;
}
...
template <>
__inline__ __device__ __ppu_bfloat16
scaled_vec_conversion<__ppu_bfloat16, uint8_t>(const uint8_t& a, float scale) {
  fp8_type f8;
  f8.__x = a;
...
template <>
__inline__ __device__ __ppu_bfloat162
scaled_vec_conversion<__ppu_bfloat162, uint16_t>(const uint16_t& a,
                                                float scale) {
  __ppu_bfloat162 res;
  res.x = scaled_vec_conversion<__ppu_bfloat16, uint8_t>((uint8_t)a, scale);
  res.y =
      scaled_vec_conversion<__ppu_bfloat16, uint8_t>((uint8_t)(a >> 8U), scale);
  return res;
}
...
__inline__ __device__ uint8_t scaled_vec_conversion<uint8_t, __ppu_bfloat16>(
    const __ppu_bfloat16& a, float scale) {
...
// bf16x2 -> fp8x2
template <>
__inline__ __device__ uint16_t scaled_vec_conversion<uint16_t, __ppu_bfloat162>(
    const __ppu_bfloat162& a, float scale) {
  union {
    uint8_t ui8[2];
    uint16_t ui16;
  } tmp;
  tmp.ui8[0] = scaled_vec_conversion<uint8_t, __ppu_bfloat16>(a.x, scale);
  tmp.ui8[1] = scaled_vec_conversion<uint8_t, __ppu_bfloat16>(a.y, scale);
  return tmp.ui16;
}
...
scaled_vec_conversion<uint32_t, bf16_4_t>(const bf16_4_t& a, float scale) {
  ...
  tmp.ui16[0] = scaled_vec_conversion<uint16_t, __ppu_bfloat162>(a.x, scale);
  tmp.ui16[1] = scaled_vec_conversion<uint16_t, __ppu_bfloat162>(a.y, scale);
  return tmp.ui32;
}
