...
class DeepEPLLPrepareAndFinalize(mk.FusedMoEPrepareAndFinalizeModular):
    ...
    # on it.
    SUPPORTED_HIDDEN_SIZES = [2048, 2560, 3072, 3584, 4096, 5120, 6144, 7168, 8192]
    ...
    def _do_quant(
        ...
        if self.use_fp8_dispatch:
            ...
            if current_platform.is_ppu() and quant_config.block_shape is None:
    ...
    def prepare_async(
        ...
        use_recv_hook = ppu_envs.VLLM_DEEPEPLL_RECV_HOOK
        (
            expert_x,
            expert_num_tokens,
            handle,
            ...
            **(
                dict(use_int8=self.use_int8_dispatch)
                if current_platform.is_ppu()
                else dict()
            ),
            **(
                dict(use_mxfp4=self.use_mxfp4_dispatch)
                if current_platform.is_ppu()
                else dict()
            ),
            **(
                dict(mxfp4_scale_row_major=False)
                if (current_platform.is_ppu() and self.use_mxfp4_dispatch)
                else dict()
            ),
            **(
                dict(
    ...
    def _finalize(
        ...
        do_recv_hook = async_path and ppu_envs.VLLM_DEEPEPLL_RECV_HOOK
