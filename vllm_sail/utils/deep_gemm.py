...
def get_deep_gemm_best_configs():
    config_dir = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "deepgemm_configs"
    )
        ...
        if (M, N, K, num_groups) in best_configs:
            ...
            if (M_candidate, N, K, num_groups) in best_configs:
                config = ast.literal_eval(
                    best_configs[(M_candidate, N, K, num_groups)]["config"]
                )
...
def _lazy_init() -> None:
    ...
    global \
        _w4a16_grouped_nopad_impl, \
        _w4a16_grouped_fused_impl, \
        _w4a16_grouped_masked_impl
    ...
    _w4a16_grouped_nopad_impl = getattr(_dg, "m_grouped_gemm_w4a16_nopad", None)
    _w4a16_grouped_fused_impl = getattr(_dg, "m_grouped_gemm_w4a16_fused", None)
    _w4a16_grouped_masked_impl = getattr(_dg, "m_grouped_gemm_w4a16_masked", None)
...
def is_ppu_15() -> bool:
    ...
    return current_platform.is_ppu() and current_platform.is_device_capability((8, 9))
...
def should_use_deepgemm_for_bf16_linear(
    ...
    Requires `VLLM_SAIL_DENSE_BF16_DEEPGEMM` and PPU 1.5; DeepGEMM dense
