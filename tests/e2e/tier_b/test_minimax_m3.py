...
# Adapted from upstream tests/kernels/test_fused_minimax_m3_qknorm_rope_kv_insert.py
...
def test_sparse_full(num_tokens, block_size, kv_cache_dtype):
    ...
    if num_tokens > 1:
        # Main and index cache padding are independent; queries still compute.
        slot_mapping[0] = -1
        index_slot_mapping[1] = -1
    ...
    written = torch.zeros(num_blocks * block_size, dtype=torch.bool, device=device)
    written[slot_mapping[slot_mapping >= 0]] = True
    untouched = kv_cache.transpose(1, 2).reshape(
        num_blocks * block_size, num_kv_heads, 2 * HEAD_DIM
    )[~written]
    torch.testing.assert_close(untouched, torch.zeros_like(untouched), rtol=0, atol=0)
    expected_index_cache = torch.zeros_like(index_cache).view(-1, HEAD_DIM)
    valid_index = index_slot_mapping >= 0
    expected_index_cache[index_slot_mapping[valid_index]] = index_k[valid_index]
    torch.testing.assert_close(
